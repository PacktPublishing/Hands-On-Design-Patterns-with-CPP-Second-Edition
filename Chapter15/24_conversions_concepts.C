...
class SmartPtr : private DeletionPolicy, public CopyMovePolicy {
    ...
    template <typename U = T> requires (ConversionPolicy::implicit_conv || ConversionPolicy::explicit_conv)
    explicit (ConversionPolicy::explicit_conv) operator U*() { return p_; }
    template <typename U = T> requires (ConversionPolicy::implicit_conv || ConversionPolicy::explicit_conv)
    explicit (ConversionPolicy::explicit_conv) operator const U*() const { return p_; }
