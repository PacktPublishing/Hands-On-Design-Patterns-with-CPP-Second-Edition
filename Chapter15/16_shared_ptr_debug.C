...
int main() {
    {
        SmartPtr<C, DeleteByOperator<C>, NoMoveNoCopy, Debug> c(new C(42));
    ...
    {
        SmallHeap h;
        SmartPtr<C, DeleteSmallHeap<C>, NoMoveNoCopy, Debug> c{new(&h) C(42), DeleteSmallHeap<C>(h)};
        std::cout << "C: " << c->get() << " @ " << &*c << std::endl;
    }

    {
        SmartPtr<C, DeleteByOperator<C>, NoMoveCopyRefCounted, Debug> c{new C(42)};
    ...
    {
        SmartPtr<C, DeleteByOperator<C>, MoveNoCopy, Debug> c{new C(42)};
        std::cout << "C: " << c->get() << " @ " << &*c << std::endl;
        auto c1(std::move(c));
