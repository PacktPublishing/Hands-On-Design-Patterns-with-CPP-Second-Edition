...
{
    //using T = V::base_type; // Does not work: V == Value is an incomplete type when its base class is instantiated
...
//template <typename T, template <typename, typename, typename...> class... Policies> // "class" -> "typename" in C++17, template must have >=2 parameters
class Value : public Policies<T, Value<T, Policies...>>...
{
    ...
    template <typename U> using rebind = Value<U, Policies...>;

    private:
    // No way to declare this a friend?
