...
int main() {
    Knight::ptr k(Knight::construct(10, 5));
    Unit::ptr o(Ogre::construct(12, 2));
    k->charge();
    cout << "Knight hits Ogre: " << k->hit(*o) << endl;
    Unit::ptr t(Troll::construct(14, 10));
    cout << "Knight hits Troll: " << k->hit(*t) << endl;
    Knight::ptr vk(VeteranUnit<Knight>::construct(k, 7, 2));
    ...
    Unit::ptr vo(VeteranUnit<Ogre>::construct(o, 1, 9));
    cout << "Veteran Knight hits Veteran Ogre: " << vk->hit(*vo) << endl;
    Unit::ptr vvo(VeteranUnit<VeteranUnit<Ogre>>::construct(vo, 1, 9));
    cout << "Veteran Knight hits Veteran2 Ogre: " << vk->hit(*vvo) << endl;
    vk->charge();
    cout << "Veteran Knight hits Veteran2 Ogre: " << vk->hit(*vvo) << endl;
    if (0) {    // Enable to see assert for casting to a wrong type (Knight to Ogre)
        Unit::ptr k(Knight::construct(10, 5));
        Unit::ptr vk(VeteranUnit<Ogre>::construct(k, 7, 2));
    }
}
